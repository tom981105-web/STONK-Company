// src/homeGate.js — STONK Home 중심 진입 게이트 (ESM 공용, Bank 와 동일)
...
export function showHomeGate({ title = "STONK Home에서 입장해 주세요", message = "", roomCode = "", auto = true } = {}) {
  ...
  Object.assign(wrap.style, {
    ...
    padding: "24px", background: "radial-gradient(120% 90% at 50% -10%, rgba(54,211,153,0.20), transparent 60%), rgba(5,6,10,0.94)",
  ...
  wrap.innerHTML = `
    <div style="width:min(460px,100%);text-align:center;padding:32px 26px;border:1px solid rgba(255,255,255,0.14);border-radius:18px;background:rgba(14,16,24,0.92);box-shadow:0 24px 70px rgba(0,0,0,0.5),0 0 60px rgba(54,211,153,0.16)">
      ...
      <p style="margin:0 0 18px;color:#aab2c8;font-size:0.95rem;line-height:1.5">${message || "로그인은 STONK Home에서 진행합니다. 같은 계정의 자산이 그대로 연결됩니다."}</p>
      <a data-home-go href="${url}" style="display:inline-flex;align-items:center;justify-content:center;min-height:50px;padding:0 26px;border-radius:14px;font-weight:900;text-decoration:none;color:#0a0a12;background:linear-gradient(135deg,#6ee7b0,#36d399);box-shadow:0 10px 30px rgba(54,211,153,0.4)">STONK Home으로 이동</a>
  ...
  if (auto2) {
    ...
    const iv = setInterval(() => { left -= 1; if (countEl) countEl.textContent = String(Math.max(0, left)); if (left <= 0) clearInterval(iv); }, 1000);
